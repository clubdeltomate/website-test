...
`
  ...
  const templates =
    opts.layoutTemplates && opts.layoutTemplates.length > 0
      ? `
SLIDE LAYOUT TEMPLATES — this is the ONLY set of slide configurations you may use. Build each slide as one of these layouts, following its component types IN ORDER and including ALL of its steps; pick the layout that best fits the concept and vary layouts across the deck. A layout with several "Text" steps means that many DISTINCT paragraphs (never repeat one). The same applies to every other repeated step: several "Image" steps mean that many DIFFERENT illustrations of the same subject — a different angle, stage, example or detail each time, each with its own alt text — and several "Graph" steps mean that many DIFFERENT charts, each showing its own data with its own title, not the same figure restated. Several "Code" steps mean the same example in different files or languages (e.g. the HTML, the CSS and the JavaScript), each labelled with its language. The player shows a run of these one page at a time, so duplicates are immediately obvious to the reader. The evaluation step names map to the quiz "kind" (see rule 6): "Multiple choice"→mcq, "2-option"→mcq2, "Fill blank"→fillblank, "Typed answer"→typed, "Solve (worksheet)"→solve. Emit the quiz with EXACTLY that kind and its required fields — do not substitute one kind for another. These layouts are already chosen for this deck's difficulty level, so honor the text density they imply.
...
ANATOMY ATLAS (read-throughs tagged [anatomy]/[medicine]/[health] with NO evaluation step): teach exactly like an anatomy book, the same way the Wolfram layouts deepen math. The image is the centrepiece — a clear, labelled sketch or depiction of the organ/structure (prefer a clean schematic sketch when a real photo would not read well); tables hold the hormones/markers/chemicals with their gland or source, target and effect (or normal ranges); charts show how a level BEHAVES — over time, across a cycle, or by dose. The surrounding text tells the reader what they are looking at, what to notice, and why it matters, so the visual genuinely deepens the chosen topic rather than decorating it.
